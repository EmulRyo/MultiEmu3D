...
class Blip_Synth {
	...
	enum {
		...
		width = (quality < 5 ? quality * 4 : Gb_Blip_Buffer::widest_impulse_),
...
public:
	...
	Gb_Blip_Buffer* output() const             { return impulse.buf; }
	void output( Gb_Blip_Buffer* b )           { impulse.buf = b; }
	...
	void offset( blip_time_t, int delta, Gb_Blip_Buffer* ) const;
	
	void offset_resampled( blip_resampled_time_t, int delta, Gb_Blip_Buffer* ) const;
	...
	void offset_inline( blip_time_t time, int delta, Gb_Blip_Buffer* buf ) const {
...
public:
	...
	Gb_Blip_Buffer* output() const         { return synth.output(); }
	void output( Gb_Blip_Buffer* b )       { synth.output( b ); if ( !b ) time_ = last_amp = 0; }
...
inline void Blip_Synth<quality,range>::offset_resampled( blip_resampled_time_t time,
		int delta, Gb_Blip_Buffer* blip_buf ) const
{
	typedef blip_pair_t_ pair_t;
	
	unsigned sample_index = (time >> BLIP_BUFFER_ACCURACY) & ~1;
    // "Blip_Synth/Blip_wave: Went past end of buffer"
	assert(sample_index < blip_buf->buffer_size_ );
	enum { const_offset = Gb_Blip_Buffer::widest_impulse_ / 2 - width / 2 };
...
template<int quality,int range>
void Blip_Synth<quality,range>::offset( blip_time_t time, int delta, Gb_Blip_Buffer* buf ) const {
	offset_resampled( time * buf->factor_ + buf->offset_, delta, buf );
}
