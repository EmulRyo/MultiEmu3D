...
private:
	// noncopyable
	Gb_Blip_Buffer( const Gb_Blip_Buffer& );
	Gb_Blip_Buffer& operator = ( const Gb_Blip_Buffer& );
...
public:
	...
	int begin( Gb_Blip_Buffer& blip_buf ) {
	...
	int read() const {
		return accum >> Gb_Blip_Buffer::accum_fract;
	}
	
	void next( int bass_shift = 9 ) {
		accum -= accum >> bass_shift;
		accum += ((long) *buf++ - Gb_Blip_Buffer::sample_offset_) << Gb_Blip_Buffer::accum_fract;
	}
	
	void end( Gb_Blip_Buffer& blip_buf ) {
...
inline int Gb_Blip_Buffer::length() const {
	return length_;
}

inline long Gb_Blip_Buffer::samples_avail() const {
	return long (offset_ >> BLIP_BUFFER_ACCURACY);
}

inline long Gb_Blip_Buffer::sample_rate() const {
	return samples_per_sec;
}

inline void Gb_Blip_Buffer::end_frame( blip_time_t t ) {
	offset_ += t * factor_;
    // "Blip_Buffer::end_frame(): Frame went past end of buffer"
	assert(samples_avail() <= (long) buffer_size_ );
}

inline void Gb_Blip_Buffer::remove_silence( long count ) {
    // "Blip_Buffer::remove_silence(): Tried to remove more samples than available"
	assert(count <= samples_avail() );
	offset_ -= blip_resampled_time_t (count) << BLIP_BUFFER_ACCURACY;
}

inline int Gb_Blip_Buffer::output_latency() const {
	return widest_impulse_ / 2;
}

inline long Gb_Blip_Buffer::clock_rate() const {
	return clocks_per_sec;
}

inline void Gb_Blip_Buffer::clock_rate( long cps )
{
	clocks_per_sec = cps;
