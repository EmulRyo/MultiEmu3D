...
bool Blip_Buffer::sample_rate( long sps, int msec )
{
	...
	//"Blip_Buffer::sample_rate(): Buffer length exceeds limit"
...
void Blip_Buffer::clock_rate( long cps ) {
    ...
    //"Blip_Buffer::clock_rate(): Clock rate to sample rate ratio exceeded 65536"
	assert(factor_ > 0);
}
...
{
    //"Blip_Buffer::read_samples(): Buffer sample rate not set"
	assert(buffer_);
