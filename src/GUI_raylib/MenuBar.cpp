...
void MenuBar::Draw(Rectangle bounds) {
    ...
    if (m_menuActive >= 0) {
        // El submen� puede solicitar que no se cierre
    ...
    // A cualquiera modulo externo que pregunte si el men� est� abierto solo se le informar� de manera negativa cuando
    // el usuario halla levantado el bot�n. Esto evita que el click se transmita a otros botones que esten debajo
    if (pendingToClose) {
