...
#include "../GB-GBC/GB.h"
#include "../GB-GBC/Def.h"
#include "../GB-GBC/Debugger.h"
...
BEGIN_EVENT_TABLE(DebuggerGBDialog, wxDialog)
EVT_BUTTON(ID_DEBUG_RESET, DebuggerGBDialog::OnReset)
EVT_BUTTON(ID_DEBUG_STEPINTO, DebuggerGBDialog::OnStepInto)
EVT_BUTTON(ID_DEBUG_ONEFRAME, DebuggerGBDialog::OnOneFrame)
EVT_BUTTON(ID_DEBUG_ONESECOND, DebuggerGBDialog::OnOneSecond)
EVT_BUTTON(ID_DEBUG_BREAKPOINTS, DebuggerGBDialog::OnBreakpoints)
EVT_TEXT(ID_DEBUG_MEMADDRESS, DebuggerGBDialog::OnMemAddressChange)
EVT_LIST_ITEM_ACTIVATED(ID_DEBUG_DISASSEMBLER, DebuggerGBDialog::OnActivated)
END_EVENT_TABLE()

DebuggerGBDialog::DebuggerGBDialog(wxWindow *parent, VideoGameDevice *device)
{
    this->Create(parent, ID_DEBUGGERDIALOG, wxT("Debugger"), wxDefaultPosition,
           wxDefaultSize, wxDEFAULT_DIALOG_STYLE);
    
    GameBoy::GB *gb = (GameBoy::GB *)device;
    m_debugger = (GameBoy::Debugger *)gb->GetDebugger();
...
DebuggerGBDialog::~DebuggerGBDialog()
{
	
}

void DebuggerGBDialog::UpdateUI() {
    UpdateRegisters();
    UpdateMemory();
    UpdateDissassembler();
    UpdateVideoRegs();
...
void DebuggerGBDialog::UpdateMemory() {
    wxString address = m_addressMemCtrl->GetValue();
    long value;
    if(address.ToLong(&value, 16)) {
        value = value & 0xFFF0;
        int numLines = 9;
        u16 maxStart = 0x10000 - (0x10*numLines);
        if (value > maxStart)
            value = maxStart;
...
void DebuggerGBDialog::UpdateRegisters() {
...
void DebuggerGBDialog::UpdateVideoRegs() {
    const char *names[]    = { "LCDC", "STAT", "SCY", "SCX", "LY", "LYC", "DMA", "BGP", "OBP0", "OBP1", "WY", "WX", "BGPI", "BGPD", "OBPI", "OBPD" };
    const u16 addresses[] = {  LCDC,   STAT,   SCY,   SCX,   LY,   LYC,   DMA,   BGP,   OBP0,   OBP1,   WY,   WX,   BGPI,   BGPD,   OBPI,   OBPD  };
...
void DebuggerGBDialog::UpdateOtherRegs() {
    const char *names[]    = { "P1", "SB", "SC", "DIV", "TIMA", "TMA", "TAC", "KEY1", "VBK", "HDMA1", "HDMA2", "HDMA3", "HDMA4", "HDMA5", "SVBK", "IF", "IE" };
    const u16 addresses[] = {  P1,   SB,   SC,   DIV,   TIMA,   TMA,   TAC,   KEY1,   VBK,   HDMA1,   HDMA2,   HDMA3,   HDMA4,   HDMA5,   SVBK,   IF,   IE  };
...
void DebuggerGBDialog::UpdateDissassembler() {
...
void DebuggerGBDialog::OnReset(wxCommandEvent &event) {
    m_debugger->Reset();
    UpdateUI();
}

void DebuggerGBDialog::OnStepInto(wxCommandEvent &event) {
    m_debugger->StepInto();
    UpdateUI();
}

void DebuggerGBDialog::OnOneFrame(wxCommandEvent &event) {
    m_debugger->ExecuteOneFrame();
    UpdateUI();
}

void DebuggerGBDialog::OnOneSecond(wxCommandEvent &event) {
    for (int i=0; i<60; i++) {
...
void DebuggerGBDialog::OnBreakpoints(wxCommandEvent &event) {
    BreakpointsDialog breakpoints(this, m_debugger);
    breakpoints.ShowModal();
    UpdateUI();
}

void DebuggerGBDialog::OnMemAddressChange(wxCommandEvent &event) {
...
void DebuggerGBDialog::OnActivated(wxListEvent &event) {
