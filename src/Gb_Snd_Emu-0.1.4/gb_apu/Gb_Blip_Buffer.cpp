...
void Gb_Blip_Buffer::clear( bool entire_buffer )
{
...
blargg_err_t Gb_Blip_Buffer::set_sample_rate( long new_rate, int msec )
{
...
blip_resampled_time_t Gb_Blip_Buffer::clock_rate_factor( long clock_rate ) const
{
...
void Gb_Blip_Buffer::bass_freq( int freq )
{
	bass_freq_ = freq;
	if ( freq == 0 )
	{
...
long Gb_Blip_Buffer::count_samples( blip_time_t t ) const
{
	return (resampled_time( t ) >> BLIP_BUFFER_ACCURACY) - (offset_ >> BLIP_BUFFER_ACCURACY);
}

blip_time_t Gb_Blip_Buffer::count_clocks( long count ) const
{
...
void Gb_Blip_Impulse_::init( blip_pair_t_* imps, int w, int r, int fb )
{
	fine_bits = fb;
	width = w;
	impulses = (imp_t*) imps;
...
void Gb_Blip_Impulse_::scale_impulse( int unit, imp_t* imp_in ) const
{
...
void Gb_Blip_Impulse_::fine_volume_unit()
{
	// to do: find way of merging in-place without temporary buffer
	
	imp_t temp [max_res * 2 * Gb_Blip_Buffer::widest_impulse_];
...
void Gb_Blip_Impulse_::volume_unit( double new_unit )
{
...
void Gb_Blip_Impulse_::treble_eq( const blip_eq_t& new_eq )
{
	...
	float buf [max_res * (Gb_Blip_Buffer::widest_impulse_ - 2) / 2];
	...
	for ( int i = size; i--; )
	{
		...
		if ( width > 12 )
		{
			double window = cos( n_harm / 1.25 / Gb_Blip_Buffer::widest_impulse_ * angle );
			y *= window * window;
		}
...
void Gb_Blip_Buffer::remove_samples( long count )
{
...
long Gb_Blip_Buffer::read_samples( blip_sample_t* out, long max_samples, bool stereo )
{
	assert( buffer_ ); // sample rate must have been set
...
void Gb_Blip_Buffer::mix_samples( const blip_sample_t* in, long count )
{
